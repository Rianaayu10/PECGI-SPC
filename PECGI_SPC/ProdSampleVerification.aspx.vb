...
#Region "DECLARATION"
    ...
    Dim ProcessGroup_Sel As String = "9"
    Dim LineGroup_Sel As String = "10"
    Dim ProcessCode_Sel As String = "11"
    Dim GetFilter As String = "12"
...
#Region "INSERT - UPDATE - DELETE ACTIVITY MONITORING CELL EDITOR"
    Private Sub Grid_CellEditorInitialize(ByVal sender As Object, ByVal e As DevExpress.Web.ASPxGridViewEditorEventArgs) Handles Grid.CellEditorInitialize
        ...
        If Grid.IsNewRowEditing Then
            ...
            End If
        ElseIf Not Grid.IsNewRowEditing Then
        ...
        If e.Column.FieldName = "PIC" Then
            ...
            If Grid.IsEditing Then Call up_FillcomboGrid(combo, HideValue.Get("LineCode")) : combo.Value = e.Value
    ...
    Protected Sub Grid_Validating(ByVal sender As Object, ByVal e As DevExpress.Web.Data.ASPxDataValidationEventArgs) Handles Grid.RowValidating
...
#Region "FUNCTION"
    ...
    Private Sub UpFillCombo()
        Try
            ...
            With cboProcessGroup
                ...
                .DataBind()
            End With
            If prmProcessGroup <> "" Then
                For i = 0 To dt.Rows.Count - 1
                    If dt.Rows(i)("CODE") = prmProcessGroup Then
            ...
            With cboLineGroup
                ...
                .DataBind()
            End With
            If prmLineGroup <> "" Then
                For i = 0 To dt.Rows.Count - 1
                    If dt.Rows(i)("CODE") = prmLineGroup Then
            ...
            With cboProcessCode
                ...
                .DataBind()
            End With
            If prmProcessCode <> "" Then
                For i = 0 To dt.Rows.Count - 1
                    If dt.Rows(i)("CODE") = prmProcessCode Then
    ...
    Private Sub LoadForm_ByAnotherform()
        ...
        prmProcessCode = dt.Rows(0)("ProcessCode").ToString
        prmLineCode = dt.Rows(0)("LineCode").ToString
        prmItemType = dt.Rows(0)("ItemTypeCode").ToString
        ...
        Dim dtVal = clsProdSampleVerificationDB.Validation(GetVerifyChartSetup, cls)
        Dim Resp = dtVal.Rows(0)("response")
        Dim RespDesc = dtVal.Rows(0)("respDesc")
        ...
        ElseIf Request.QueryString("menu") = "SPCDashboard.aspx" Then
            btnBack.Visible = True
        ElseIf Request.QueryString("menu") = "AlertDelayVerification.aspx" Then
            HideValue.Set("prm_factory", prmFactoryCode)
            HideValue.Set("prm_FilterDate", Request.QueryString("FilterDate"))
            btnBack.Visible = True
        End If
    ...
    Private Sub GetURL(cls As clsProdSampleVerification)
        ...
        Dim dtIOT_ProcessTable = clsIOT.GetIOT_DailyProd(cls.FactoryCode, ProcessTableLineCode, cls.ItemType_Code, cls.ProdDate, cls.ShiftCode)
        If dtIOT_ProcessTable.Rows.Count > 0 Then
            ...
            InstructionNo = dtIOT_ProcessTable.Rows(0)("InstructionNo").ToString.Trim
            LotNo = dtIOT_ProcessTable.Rows(0)("LotNo").ToString.Trim
        End If
    ...
    Private Sub GridTitle(ByVal pExl As ExcelWorksheet, cls As clsProdSampleVerification)
        With pExl
            Try
                ...
                .Cells(irow, 2).Value = ": " & cls.FactoryName
                .Cells(irow, 4).Value = "Machine"
                .Cells(irow, 5).Value = ": " & cls.ProcessName
                .Cells(irow, 7).Value = "Item Check"
                .Cells(irow, 8).Value = ": " & cls.ItemCheck_Name
                .Cells(irow, 11).Value = "Show Verified Only : " & cls.ShowVerify_Desc
                irow = irow + 1

                .Cells(irow, 1).Value = "Process Group"
                .Cells(irow, 2).Value = ": " & cls.ProcessGroupName
                .Cells(irow, 4).Value = "Machine Process"
                .Cells(irow, 5).Value = ": " & cls.LineName
                .Cells(irow, 7).Value = "Prod. Date"
                .Cells(irow, 8).Value = ": " & cls.Period
                irow = irow + 1

                .Cells(irow, 1).Value = "Line Group"
                .Cells(irow, 2).Value = ": " & cls.LineGroupName
                .Cells(irow, 4).Value = "Type"
                .Cells(irow, 5).Value = ": " & cls.ItemType_Name
                .Cells(irow, 7).Value = "Shift"
                .Cells(irow, 8).Value = ": " & cls.ShiftName
                .Cells(irow, 9).Value = "Seq :" & cls.Seq
                irow = irow + 1
                ...
                Dim rgHeader As ExcelRange = .Cells(irowStar, 1, irow, 17)
            ...
            With pExl
                ...
                If dtGrid.Rows.Count > 0 Then
                    ...
                    For i = 0 To dtGrid.Rows.Count - 1
                        ...
                        .Cells(irow + i, 7, irow + i, 8).Value = dtGrid.Rows(i)("Remark")

...
/****** Object:  StoredProcedure [dbo].[SP_SPC_ProdSampleVerification_Grid]    Script Date: 1/20/2023 4:41:36 PM ******/
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			SET @query += N' SELECT nDescIndex, nDesc,' + @cols + N' from 
						 (
							SELECT nTime, nDesc = ''X Bar'' ,nAVG = CAST(nAVG AS VARCHAR(21)), nDescIndex = ''XBar'' 
							FROM #TempXbar
						) A
						PIVOT 
						(
			...
			SET @query += N' SELECT nDescIndex, nDesc,' + @cols + N' from 
						 (
							SELECT nTime, nDesc = ''MK/GC/SC'' , MK, nDescIndex = ''Verification'' 
							FROM #TempXbar
						) A
						PIVOT 
						(

USE [PECGI_SPC]
GO
/****** Object:  StoredProcedure [dbo].[SP_SPC_ProdSampleVerification_Grid]    Script Date: 12/9/2022 6:04:46 PM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	BEGIN
		...
		FROM (
			...
			AND ProdDate IN (@ProdDate_From,@ProdDate_To) 
		) A ORDER BY nDate
	END
	/*=============================================================================*/
	...
	BEGIN
		...
		AND ProdDate = CAST(@ProdDate_Grid AS DATE)
		ORDER BY ShiftCode
	END
	/*=============================================================================*/
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				AND ItemCheckCode = @pItemCheckCode AND ProdDate < @pProdDate

				SELECT @Last_Seq = MAX(SequenceNo) FROM spc_Result A
				JOIN (SELECT SPCResultID FROM spc_ResultDetail WHERE DeleteStatus <> '1' GROUP BY SPCResultID
				) B ON A.SPCResultID = B.SPCResultID
				WHERE FactoryCode = @pFactoryCode and ItemTypeCode = @pItemTypeCode and LineCode = @pLineCode 
				AND ItemCheckCode = @pItemCheckCode AND ProdDate = @Last_ProdDate
...
END
GO
/****** Object:  StoredProcedure [dbo].[SP_SPC_ProdSampleVerification_Verify]    Script Date: 12/9/2022 6:04:46 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
-- Description	: <->
-- =============================================
CREATE PROCEDURE [dbo].[SP_SPC_ProdSampleVerification_Verify]
	@FactoryCode AS VARCHAR(25),	
	@ItemTypeCode AS VARCHAR(25),
	@LineCode AS VARCHAR(5),
	@ItemCheckCode AS VARCHAR(15),
...
AS
BEGIN
	DECLARE   @RegDate As DATETIME = GETDATE()
			, @UserPosition AS CHAR(2) = ''
			, @Result AS VARCHAR(250) = ''
			, @MKVerify AS VARCHAR(50) = ''
			, @QCVerify AS VARCHAR(50) = ''

	IF EXISTS ( SELECT B.SampleSize, C.nValue FROM spc_Result A
				JOIN spc_ItemCheckByType B ON A.FactoryCode = B.FactoryCode AND A.ItemTypeCode = B.ItemTypeCode AND A.LineCode = B.LineCode AND A.ItemCheckCode = B.ItemCheckCode			
				LEFT JOIN ( SELECT COUNT(C.SPCResultID) AS nValue, A.SPCResultID
							FROM spc_Result A
							JOIN spc_ChartSetup B ON A.FactoryCode = B.FactoryCode AND A.ItemTypeCode = B.ItemTypeCode
								AND A.LineCode = B.LineCode AND A.ItemCheckCode = B.ItemCheckCode 
								AND (A.ProdDate BETWEEN CAST(B.StartDate AS DATE) AND CAST(B.EndDate AS DATE))
							JOIN spc_ResultDetail C ON A.SPCResultID = C.SPCResultID AND ISNULL(C.DeleteStatus,'0') <> '1'
							WHERE C.VALUE >= B.SpeclSL AND C.Value <= B.SpecUSL
							GROUP BY A.SPCResultID ) AS C ON A.SPCResultID = C.SPCResultID
				WHERE A.FactoryCode = @FactoryCode 
					AND A.ItemTypeCode = @ItemTypeCode
					AND A.LineCode = @LineCode 
					AND A.ItemCheckCode = @ItemCheckCode
					AND A.ProdDate = CAST(@ProdDate AS DATE)
					AND A.ShiftCode = @ShiftCode
					AND A.SequenceNo = @Seq			
					AND nValue >= SampleSize				
			 )
	BEGIN
		SELECT @UserPosition = JobPosition FROM spc_UserSetup Where UserID = @User 
		SELECT @MKVerify = ISNULL(MKVerificationUser,''), @QCVerify = ISNULL(QCVerificationUser,'')
		...
		BEGIN
			IF @MKVerify = ''
			BEGIN
					UPDATE dbo.spc_Result
					SET MKVerificationUser = @User, MKVerificationStatus = '1', MKVerificationDate = @RegDate
					WHERE FactoryCode = @FactoryCode 
					AND ItemTypeCode = @ItemTypeCode
					AND LineCode = @LineCode 
					AND ItemCheckCode = @ItemCheckCode
					AND ProdDate = CAST(@ProdDate AS DATE)
					AND ShiftCode = @ShiftCode
					AND SequenceNo = @Seq	
				END
				ELSE
				BEGIN
					SET @Result = 'The Data Already Verified!'
				END

			END

			ELSE IF @UserPosition = 'QC' 
			BEGIN
				IF @MKVerify = ''
				BEGIN
					SET @Result = 'Can Not Verify The Data, Please Verify from Machine Kipper first!'
				END
				ELSE IF @QCVerify <> '' 
				BEGIN		
					SET @Result = 'The Data Already Verified!'
				...
				BEGIN
					UPDATE dbo.spc_Result
					SET QCVerificationUser = @User, QCVerificationStatus = '1', QCVerificationDate = @RegDate
					WHERE FactoryCode = @FactoryCode 
					AND ItemTypeCode = @ItemTypeCode
					AND LineCode = @LineCode 
					AND ItemCheckCode = @ItemCheckCode
			...
			ELSE
			BEGIN
				SET @Result = 'Can Not Verify The Data, User Not Have Privilege to Verify!'
			END
		
			
		--IF NOT EXISTS (SELECT * FROM spc_Result A
		--				JOIN (
		--					SELECT DISTINCT SPCResultID FROM spc_ResultDetail WHERE DeleteStatus <> '1'
		--				) AS B ON A.SPCResultID = B.SPCResultID
		--				WHERE FactoryCode = @FactoryCode 
		--					AND ItemTypeCode = @ItemTypeCode
		--					AND LineCode = @LineCode 
		--					AND ItemCheckCode = @ItemCheckCode
		--					AND ProdDate <= CAST(@ProdDate AS DATE)
		--					AND ShiftCode = @ShiftCode
		--					AND 1 = CASE WHEN ProdDate = CAST(@ProdDate AS DATE) AND SequenceNo < @Seq AND (ISNULL(MKVerificationUser,'') = '' OR ISNULL(QCVerificationUser,'') = '') THEN 1 
		--							WHEN ProdDate <> CAST(@ProdDate AS DATE) AND (ISNULL(MKVerificationUser,'') = '' OR ISNULL(QCVerificationUser,'') = '') THEN 1
		--							ELSE 0 END )
		--BEGIN
		--END
		--ELSE
		--BEGIN
		--	SET @Result = 'Can not verify, Please do verify the previous sequence first'
		--END	
	END
	ELSE
	BEGIN
		SET @Result = 'Please re-measure the sample, Verification process is failed'
	END

	SELECT Result = @Result
END
GO
/****** Object:  StoredProcedure [dbo].[SP_SPC_ProdSampleVerificationList_Grid]    Script Date: 12/9/2022 6:04:46 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
BEGIN
	...
	LEFT JOIN ( SELECT ShowVerify = CASE WHEN A.SampleSizeOK < SampleSize THEN '0' ELSE '1' END, A.SPCResultID
				FROM (	SELECT SampleSizeOK = COUNT(C.SPCResultID), ICT.SampleSize, A.SPCResultID 
						...
						WHERE C.DeleteStatus <> '1' AND C.Value > SpecLSL AND C.Value < SpecUSL 
	...
	FROM ( SELECT DISTINCT
			...
			LEFT JOIN (SELECT nMax = MAX([Value])
							...
							WHERE ISNULL(A.DeleteStatus,'0') = '0' 
							AND 1 = CASE WHEN E.ShowVerify = '1' AND (Value > SpecUSL OR Value < SpecLSL) THEN 0 ELSE 1 END
	...
	, ShowVerify, VerifyStatus, ProductionDate)
